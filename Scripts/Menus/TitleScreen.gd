...
func _process(_delta):
	if MusicController.playing == true:
		...
		if round(MusicController.GetBeatTime()) == 0:
			if prevent == false:
				$logo/Tween.interpolate_property($logo, "rotation_degrees", -rotateamount, rotateamount, 0.5, Tween.TRANS_QUAD, Tween.EASE_IN_OUT)
				$logo/Tween.start()
				prevent = true
		else:
			if prevent == true:
